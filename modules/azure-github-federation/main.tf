...
resource "azuread_service_principal" "default" {
  for_each       = local.app_registrations_map
  application_id = azuread_application.default[each.value.name].application_id
  owners         = length(each.value.owners) == 0 ? [data.azuread_client_config.current.object_id] : concat([data.azuread_client_config.current.object_id], each.value.owners)
}
resource "azurerm_role_assignment" "default" {
  ...
  principal_id         = azuread_service_principal.default[each.value.app].object_id
}
resource "azuread_application_federated_identity_credential" "default" {
  for_each              = local.federated_credentials_map
  application_object_id = azuread_application.default[each.value.app].object_id
